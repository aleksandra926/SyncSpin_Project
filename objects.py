...
import random
import math
from shapes import get_shape_position
...
class Balls(pygame.sprite.Sprite):
	def __init__(self, pos, radius, angle, win, shape="circle"):
		...
		self.pos_list = []  # List to store path points
		self.max_path_length = 60

	def update(self, color):
		x, y = get_shape_position(self.shape, self.angle, self.radius)
		...
		if len(self.pos_list) > self.max_path_length:
			self.pos_list.pop(0)
...
class Button(pygame.sprite.Sprite):
	def __init__(self, img, scale, x, y, enabled=True):
